...
document.getElementById('keyboard-table').addEventListener('click',
  handleLetterChoice);
...
function render() {
  ...
  renderGallows();
  guessWordEl.textContent = badLetters.length === 7 ? secretWord.join('') :
    guessWord.join('');
  document.getElementById('keyboard-row').style.display = (badLetters.length ===
    7) || (secretWord.join('') === guessWord.join('')) ? 'none' : '';
}
